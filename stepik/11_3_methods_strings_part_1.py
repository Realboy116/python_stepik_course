...
# print(s) 
'''Суммы двух
На вход программе подается натуральное число n≥2, а затем n целых чисел. Напишите программу, которая создает из 
указанных чисел список, состоящий из сумм соседних чисел (
0 и 1, 1 и 2, 2 и 3 и т.д.).
На вход программе подаются натуральное число n, а затем n целых чисел, каждое на отдельной строке.
Программа должна вывести список, состоящий из сумм соседних чисел.'''
# n, n1, s = int(input()), int(input()), []
# for i in range(n - 1):
#     n2 = int(input())
#     s.append(n1 + n2)
#     n1 = n2
...
'''ввод данных(n) согласно условию(число)
создаем пустой список(cloud = [ ])
создаем пустую строку(string = ' ')
цикл (for) c диапазоном(n)
    ввод данных(n) согласно условию(строка)
    добавляем строку(n) в список(cloud.append(n))
ввод данных(k) согласно условию(число)
цикл (for) c диапазоном(len(cloud))   
    заводим переменную(m = cloud[i]) # достаем элемент(строку) из списка
    условие( if) если (k <= len(m)): # сравниваем (к) и длину строки
          заводим переменную(x = m[k - 1]) # позволяет извлечь элемент строки по индексу
          добавляем переменную в строку(string += x)
печать строки'''
# n = int(input())
# list1 = []
# str1 = ''
